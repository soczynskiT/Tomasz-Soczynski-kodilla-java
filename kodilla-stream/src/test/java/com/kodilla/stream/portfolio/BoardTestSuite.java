...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        final List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        final long daysSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(f -> f.getTasks().stream())
                .map(t -> LocalDate.from(t.getCreated()).until(LocalDate.now(), ChronoUnit.DAYS))
                .reduce((long) 0, (sum, current) -> sum += current);
        final long tasksNo = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .mapToLong(f -> f.getTasks().size())
                .sum();
    ...
    public void testAddTaskListAverageWorkingOnTask2() {
        ...
        double average = project.getTaskLists().stream()
                ...
                .mapToLong(d -> LocalDate.from(d.getCreated()).until(LocalDate.now(), ChronoUnit.DAYS))
                .average().getAsDouble();
        //Then
        Assert.assertTrue(average == 10);
    }
